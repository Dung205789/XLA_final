...
from utils.assigner import IoUAssigner
...
def build_anchors(args, n_anchors_per_level: int = 3):
    """Run k-means (or fall back to defaults) and return anchors tensor + per-level whs."""
    ...
    try:
        ...
        all_whs = DEFAULT_ANCHOR_WHS
        anchor_whs_per_level = [
            all_whs[i * n_anchors_per_level: (i + 1) * n_anchors_per_level]
            for i in range(len(STRIDES))
        ]

    # Save anchors for inference
    ...
    return anchors, anchor_whs_per_level
...
def parse_args():
    ...
    p.add_argument("--no_kmeans", action="store_true", help="Skip k-means and use default anchors")
    return p.parse_args()
...
def main():
    ...
    if args.no_kmeans:
        ...
        anchors, anchor_whs_per_level = build_anchors(args)

    n_anchors = len(anchor_whs_per_level[0])

    # Datasets
    train_ds = DetectionDataset(args.train_data, args.image_dir, TrainTransform(args.input_size))
    ...
    assigner = IoUAssigner(pos_iou=0.5, neg_iou=0.4)
